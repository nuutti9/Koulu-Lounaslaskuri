...
export default function MenuViewer({ schoolId, schoolName }: Props) {
  ...
  // Save school choice to localStorage so homepage can redirect
  useEffect(() => {
    if (typeof window !== "undefined") {
      localStorage.setItem("kr_school_id", schoolId);
    }
  }, [schoolId]);
  ...
  // Fetch menu data when school changes
  useEffect(() => {
    let cancelled = false;
    ...
    fetch(`/api/menu?school=${encodeURIComponent(schoolId)}`)
      .then(res => res.json())
      .then(data => {
        if (!cancelled) {
    ...
    <>
      ...
      <header className={isHeaderScrolled ? "scrolled" : ""}>
        ...
        <div className="header-content" style={{ position: "relative" }}>
          <div className="header-titles">
            ...
            <h1>
              <a href="/?valitse=1" className="school-header-selector" style={{ textDecoration: "none", color: "inherit" }}>
      ...
      >
        <div className="container">
          <div className={`hero ${isLoadingMenu ? 'skeleton' : ''}`}>
          ...
          <div className={`section-label ${isLoadingMenu ? 'skeleton' : ''}`}>Päivän valikko</div>
          <div className={`list-group ${isLoadingMenu ? 'skeleton' : ''}`}>
            {isLoadingMenu ? (
              <>
          ...
          <div className={`section-label ${isLoadingMenu ? 'skeleton' : ''}`} style={{ marginTop: 24 }}>AI-analyysi</div>
          ...
          <div className="ai-zone" style={{ position: "relative", overflow: "hidden", minHeight: (!user || (hasSubscription === false && freeAnalysesUsed >= 3)) ? 320 : "auto" }}>
            ...
            <div className="ai-toolbar">
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <button className={`btn-primary ${isAnalyzing ? 'loading' : ''}`} onClick={isAnalyzing ? confirmCancelAnalysis : runAI}>Analysoi</button>
              </div>
            </div>
          </div>
